        ...
        <div className="lg:pr-8 pt-12 lg:pt-0">
          <div className="max-w-md mx-auto sm:max-w-lg lg:mx-0 text-center lg:text-left">
            ...
            <div className="flex justify-center lg:justify-start">
              <Badge variant="secondary" className="inline-flex items-center gap-2 mb-6 px-4 py-3 text-sm sm:text-base font-medium bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white border border-orange-300/50 rounded-full shadow-lg backdrop-blur-sm transition-all duration-300 hover:shadow-xl">
                <Heart className="w-6 h-6 sm:w-5 sm:h-5 flex-shrink-0 text-white" />
                <span className="text-white font-medium">United for Dharma, Dedicated for Nation</span>
              </Badge>
            </div>
            ...
            <p className="mt-6 text-base font-normal leading-7 text-muted-foreground">
              Join us in our mission to restore traditional values and build a stronger nation through dedicated service.
            </p>
            
            <div className="mt-8">
              <p className="text-base font-bold text-foreground mb-6">Our Growing Community</p>
            ...
            <div className="flex flex-row gap-3 sm:gap-4 mt-8 justify-center lg:justify-start">
              <Button 
                ...
                asChild
              >
                <Link href="https://app.joinrss.org.in/registration" className="flex items-center justify-center text-white">
                  Join Now
              ...
              {heroSlides.map((slide) => (
                <div key={slide.id} className="flex-shrink-0">
                  <div className="relative flex flex-col overflow-hidden transition-all duration-200 transform bg-background border border-border shadow w-60 md:w-80 h-[500px] group rounded-xl hover:shadow-lg hover:-translate-y-1 cursor-pointer">
                    <a href={slide.ctaLink} className="flex shrink-0 cursor-pointer">
                      <Image 
                        src={slide.image} 
                        alt={slide.title}
                        width={320}
                    ...
                    <div className="flex-1 px-4 py-5 sm:p-6 flex flex-col">
                      <a href={slide.ctaLink} className="flex-1 cursor-pointer">
                        <p className="text-lg font-bold text-foreground line-clamp-2">{slide.title}</p>
                        <p className="text-base font-semibold text-primary/80 font-serif mt-2 line-clamp-1">{slide.titleHindi}</p>
                        <p className="mt-3 text-sm font-normal leading-6 text-muted-foreground line-clamp-3">{slide.description}</p>
                      </a>
                    </div>
                    <div className="px-4 py-4 border-t border-border sm:px-6 mt-auto">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          <p className="text-sm font-medium text-foreground">RSS Service</p>
                          <span className="text-sm font-medium text-muted-foreground">•</span>
                          <p className="text-sm font-medium text-muted-foreground">Join Now</p>
                        </div>
