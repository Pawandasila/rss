...
export default function BlogPage() {
  ...
  const mappedBlogs = useMemo(() => {
    ...
    return blogs.map((blog) => ({
      id: blog.id,
      title: blog.title,
      excerpt: blog.headline || "No description available",
  ...
  const blogCategories = useMemo(() => {
    const categories = new Set(mappedBlogs.map((b) => b.category));
  ...
  const filteredPosts =
    selectedCategory === "All"
      ? mappedBlogs
      : mappedBlogs.filter((post) => post.category === selectedCategory);

  const featuredPosts = mappedBlogs.filter((post) => post.featured);
  const latestPost =
    featuredPosts.length > 0 ? featuredPosts[0] : mappedBlogs[0]; // Fallback to first post if no featured

  if (isLoadingBlogs) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background mt-10">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-6">
              ...
              <p className="text-lg sm:text-xl text-gray-600 leading-relaxed">
                राष्ट्रीय सेवा संघ की गतिविधियों, कार्यक्रमों और महत्वपूर्ण
      ...
      <section className="py-10 px-4 bg-white border-b">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">अपनी रुचि के अनुसार लेख खोजें</p>
          </div>
