...
import { useEffect, useState, useRef, type ChangeEvent, Suspense } from "react";
...
const BecomeMemberPage = () => {
  ...
  useEffect(() => {
    ...
    setFormState((prev) => {
      ...
      if (!updated.profession && maybeProfession) {
        updated.profession = maybeProfession.toLowerCase().replace(/\s+/g, "-");
      }
  ...
  const validateStep = (stepIndex: number): boolean => {
    ...
    if (stepIndex === 0) {
      ...
      if (!formState.blood_group?.trim()) {
        stepErrors.blood_group = "Blood group is required.";
      } else if (!/^(O|A|B|AB)[+-]$/.test(formState.blood_group)) {
        stepErrors.blood_group = "Please select a valid blood group.";
      }
    }
    ...
    if (stepIndex === 1) {
      ...
      if (!formState.mandal.trim()) {
        stepErrors.mandal = "Mandal is required.";
      }
    ...
    if (stepIndex === 3) {
      ...
      if (!formState.declaration_accepted) {
        stepErrors.declaration_accepted =
          "You must accept the terms and declaration to proceed.";
      }
    }
...
function BecomeMemberPageWrapper() {
  return (
    <Suspense
      fallback={
...
export default BecomeMemberPageWrapper;
