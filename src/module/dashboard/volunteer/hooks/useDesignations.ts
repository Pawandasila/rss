...
export const useDesignations = (levelName?: string, wingName?: string) => {
  ...
  const fetchDesignations = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await api.getDesignations(levelName, wingName, page, search);
  ...
  }, [levelName, wingName, page, search]);
  ...
  const createDesignation = useCallback(
    async (data: DesignationFormData) => {
      try {
        ...
        fetchDesignations(); // Refetch to look at correct page placement
  ...
  const deleteDesignation = useCallback(
    async (id: number) => {
      try {
        ...
        // Ideally refetch if the page becomes empty, but this is acceptable for now
    ...
    [designations.length, page, fetchDesignations]
  );
