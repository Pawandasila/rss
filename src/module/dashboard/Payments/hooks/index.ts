...
const normalizePayment = (payment: Record<string, unknown>): Payment => ({
  ...
  payment_details: payment.payment_details
    ...
    : null,
    is_manual: Boolean(payment.is_manual ?? false),
});
...
export function usePayments(page: number = 1, page_size: number = 20) {
  ...
  const updatePayment = useCallback(
    async (
      ...
      try {
        const response = await axios.put(`/payment/edit/${paymentId}/`, data);
  ...
  const deletePayment = useCallback(
    async (paymentId: number): Promise<{ success: boolean; message?: string }> => {
      try {
        const response = await axios.delete(`/payment/detail/${paymentId}/`);
...
const normalizeStats = (raw: Record<string, unknown>): PaymentStats => ({
  ...
  today_transactions: Number(
    raw?.today_transactions ?? (raw as Record<string, unknown>)?.todayTransactions ?? 0
  ),
});

export function usePaymentStats(date?: string) {
