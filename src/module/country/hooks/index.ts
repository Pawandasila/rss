"use client";

import { useEffect, useMemo, useState, useCallback } from "react";
import useSWR, { mutate as globalMutate } from "swr";
import useSWRImmutable from "swr/immutable";
import useAxios from "@/hooks/use-axios";
import { useAuth } from "@/hooks/use-auth";
import { toast } from "sonner";

interface CountryItem {
  name: string;
  code?: string;
}

interface StateItem {
  id?: number;
  district_count?: number;
  name: string;
}

interface DistrictItem {
  id?: number;
  name: string;
  state?: number;
  state_id?: number;
}

interface MandalItem {
  id?: number;
  name: string;
  district?: number;
  district_id?: number;
}

interface CreateStatePayload {
  name: string;
}

interface CreateDistrictPayload {
  name: string;
  state: number;
}

interface CreateMandalPayload {
  name: string;
  district: number;
}

interface UseCountryApiReturn {
  countries: CountryItem[];
  states: StateItem[];
  districts: DistrictItem[];
  mandals: MandalItem[];

  isLoadingCountries: boolean;
  isLoadingStates: boolean;
  isLoadingDistricts: boolean;
  isLoadingMandals: boolean;
  isCreatingState: boolean;
  isCreatingDistrict: boolean;
  isCreatingMandal: boolean;

  countriesError: string | null;
  statesError: string | null;
  districtsError: string | null;
  mandalsError: string | null;
  createStateError: string | null;
  createDistrictError: string | null;
  createMandalError: string | null;

  canCreateState: boolean;
  canCreateDistrict: boolean;
  canCreateMandal: boolean;

  fetchCountries: () => Promise<void>;
  fetchStates: () => Promise<void>;
  fetchDistricts: (stateId?: number) => Promise<void>;
  fetchMandals: (districtId?: number) => Promise<void>;
  refreshCountries: () => Promise<void>;
  refreshStates: () => Promise<void>;
  refreshDistricts: (stateId?: number) => Promise<void>;
  refreshMandals: (districtId?: number) => Promise<void>;

  createState: (payload: CreateStatePayload) => Promise<StateItem | null>;
  createDistrict: (
    payload: CreateDistrictPayload
  ) => Promise<DistrictItem | null>;
  createMandal: (payload: CreateMandalPayload) => Promise<MandalItem | null>;
  deleteState: (id: number) => Promise<boolean>;
  deleteDistrict: (id: number) => Promise<boolean>;
  deleteMandal: (id: number) => Promise<boolean>;

  resetStates: () => void;
  resetDistricts: () => void;
  resetMandals: () => void;
  clearErrors: () => void;
}

export function useCountryApi(): UseCountryApiReturn {
  const { isAdmin, isStaff } = useAuth();
  const axios = useAxios();

  const fetcher = useMemo(
    () => (url: string) => axios.get(url).then((r) => r.data),
    [axios]
  );

  const [isCreatingState, setIsCreatingState] = useState(false);
  const [isCreatingDistrict, setIsCreatingDistrict] = useState(false);
  const [isCreatingMandal, setIsCreatingMandal] = useState(false);

  const [createStateError, setCreateStateError] = useState<string | null>(null);
  const [createDistrictError, setCreateDistrictError] = useState<string | null>(
    null
  );
  const [createMandalError, setCreateMandalError] = useState<string | null>(
    null
  );

  const { data: countriesData, error: countriesErr } = useSWRImmutable<
    CountryItem[]
  >("countries-static", async () => [{ name: "India" }]);
  const countries = countriesData ?? [];
  const isLoadingCountries = !countriesData && !countriesErr;
  const countriesError = countriesErr ? (countriesErr as Error).message : null;

  const STATES_KEY = "/dashboard/states" as const;
  const {
    data: statesData,
    error: statesErr,
    isLoading: loadingStates,
    mutate: mutateStates,
  } = useSWR<StateItem[] | { results: StateItem[] }>(STATES_KEY, fetcher, {
    onError: (error) => {
      const errorMessage =
        error instanceof Error ? error.message : "Failed to load states";
      toast.error(errorMessage);
      console.error("Error loading states:", error);
    },
    revalidateOnFocus: false,
    shouldRetryOnError: false,
  });
  const states = Array.isArray(statesData)
    ? statesData
    : (statesData as { results?: StateItem[] })?.results ?? [];
  const statesError = statesErr ? (statesErr as Error).message : null;
  const isLoadingStates = loadingStates;

  const [selectedStateId, setSelectedStateId] = useState<number | undefined>(
    undefined
  );
  const districtsKey = selectedStateId
    ? `/dashboard/districts/?state=${selectedStateId}`
    : null;
  const {
    data: districtsData,
    error: districtsErr,
    isLoading: loadingDistricts,
    mutate: mutateDistricts,
  } = useSWR<DistrictItem[] | { results: DistrictItem[] } | undefined>(
    districtsKey,
    fetcher,
    {
      onError: (error) => {
        const errorMessage =
          error instanceof Error ? error.message : "Failed to load districts";
        toast.error(errorMessage);
        console.error("Error loading districts:", error);
      },
      revalidateOnFocus: false,
      shouldRetryOnError: false,
    }
  );
  const districts = Array.isArray(districtsData)
    ? districtsData
    : (districtsData as { results?: DistrictItem[] })?.results ?? [];
  const districtsError = districtsErr ? (districtsErr as Error).message : null;
  const isLoadingDistricts = loadingDistricts;

  // Mandal Logic
  const [selectedDistrictId, setSelectedDistrictId] = useState<
    number | undefined
  >(undefined);
  const mandalsKey = selectedDistrictId
    ? `/dashboard/division/?district=${selectedDistrictId}`
    : null;
  const {
    data: mandalsData,
    error: mandalsErr,
    isLoading: loadingMandals,
    mutate: mutateMandals,
  } = useSWR<MandalItem[] | { results: MandalItem[] } | undefined>(
    mandalsKey,
    fetcher,
    {
      onError: (error) => {
        const errorMessage =
          error instanceof Error ? error.message : "Failed to load mandals";
        toast.error(errorMessage);
        console.error("Error loading mandals:", error);
      },
      revalidateOnFocus: false,
      shouldRetryOnError: false,
    }
  );
  const mandals = Array.isArray(mandalsData)
    ? mandalsData
    : (mandalsData as { results?: MandalItem[] })?.results ?? [];
  const mandalsError = mandalsErr ? (mandalsErr as Error).message : null;
  const isLoadingMandals = loadingMandals;

  const canCreateState = isAdmin() || isStaff();
  const canCreateDistrict = isAdmin() || isStaff();
  const canCreateMandal = isAdmin() || isStaff();

  const fetchCountries = useCallback(async () => {}, []);
  const doRefreshCountries = useCallback(async () => {
    await globalMutate("countries-static");
  }, []);

  const fetchStates = useCallback(async () => {
    await mutateStates();
  }, [mutateStates]);
  const doRefreshStates = useCallback(async () => {
    await mutateStates();
  }, [mutateStates]);

  const fetchDistricts = useCallback(
    async (stateId?: number) => {
      setSelectedStateId(stateId);
      if (stateId) await mutateDistricts();
    },
    [mutateDistricts]
  );
  const doRefreshDistricts = useCallback(
    async (stateId?: number) => {
      setSelectedStateId(stateId);
      if (stateId) await mutateDistricts();
    },
    [mutateDistricts]
  );

  const fetchMandals = useCallback(
    async (districtId?: number) => {
      setSelectedDistrictId(districtId);
      if (districtId) await mutateMandals();
    },
    [mutateMandals]
  );
  const doRefreshMandals = useCallback(
    async (districtId?: number) => {
      setSelectedDistrictId(districtId);
      if (districtId) await mutateMandals();
    },
    [mutateMandals]
  );

  const createState = async (
    payload: CreateStatePayload
  ): Promise<StateItem | null> => {
    if (!canCreateState) {
      const errorMessage =
        "You do not have permission to create states. Only admin and staff users can perform this action.";
      setCreateStateError(errorMessage);
      console.error("Permission denied: User cannot create states");
      return null;
    }

    setIsCreatingState(true);
    setCreateStateError(null);

    try {
      const response = await axios.post("/admin/states/", payload);
      const newState = response.data;
      await mutateStates();
      return newState;
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Failed to create state";
      setCreateStateError(errorMessage);
      console.error("Error creating state:", error);
      return null;
    } finally {
      setIsCreatingState(false);
    }
  };

  const createDistrict = async (
    payload: CreateDistrictPayload
  ): Promise<DistrictItem | null> => {
    if (!canCreateDistrict) {
      const errorMessage =
        "You do not have permission to create districts. Only admin and staff users can perform this action.";
      setCreateDistrictError(errorMessage);
      console.error("Permission denied: User cannot create districts");
      return null;
    }

    setIsCreatingDistrict(true);
    setCreateDistrictError(null);

    try {
      const response = await axios.post("/admin/districts/", payload);
      const newDistrict = response.data;
      await globalMutate(`/dashboard/districts/?state=${payload.state}`);
      return newDistrict;
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Failed to create district";
      setCreateDistrictError(errorMessage);
      console.error("Error creating district:", error);
      return null;
    } finally {
      setIsCreatingDistrict(false);
    }
  };

  const createMandal = async (
    payload: CreateMandalPayload
  ): Promise<MandalItem | null> => {
    if (!canCreateMandal) {
      const errorMessage =
        "You do not have permission to create mandals. Only admin and staff users can perform this action.";
      setCreateMandalError(errorMessage);
      console.error("Permission denied: User cannot create mandals");
      return null;
    }

    setIsCreatingMandal(true);
    setCreateMandalError(null);

    try {
      const response = await axios.post("/admin/divisions/", payload);
      const newMandal = response.data;
      await globalMutate(`/dashboard/division/?district=${payload.district}`);
      return newMandal;
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Failed to create mandal";
      setCreateMandalError(errorMessage);
      console.error("Error creating mandal:", error);
      return null;
    } finally {
      setIsCreatingMandal(false);
    }
  };

  const resetStates = useCallback((): void => {
    setSelectedStateId(undefined);
    globalMutate(STATES_KEY, [], false);
  }, []);

  const resetDistricts = useCallback((): void => {
    setSelectedStateId(undefined);
  }, []);

  const resetMandals = useCallback((): void => {
    setSelectedDistrictId(undefined);
  }, []);

  const deleteState = useCallback(
    async (id: number): Promise<boolean> => {
      if (!canCreateState) {
        toast.error("You do not have permission to delete states.");
        return false;
      }
      try {
        await axios.delete(`/admin/states/${id}/`);
        await mutateStates();
        if (selectedStateId === id) {
          setSelectedStateId(undefined);
        }
        return true;
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "Failed to delete state";
        toast.error(message);
        console.error("Error deleting state:", error);
        return false;
      }
    },
    [axios, mutateStates, canCreateState, selectedStateId]
  );

  const deleteDistrict = useCallback(
    async (id: number): Promise<boolean> => {
      if (!canCreateDistrict) {
        toast.error("You do not have permission to delete districts.");
        return false;
      }
      try {
        await axios.delete(`/admin/districts/${id}/`);
        if (selectedStateId) {
          await mutateDistricts();
        }
        return true;
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "Failed to delete district";
        toast.error(message);
        console.error("Error deleting district:", error);
        return false;
      }
    },
    [axios, mutateDistricts, canCreateDistrict, selectedStateId]
  );

  const deleteMandal = useCallback(
    async (id: number): Promise<boolean> => {
      if (!canCreateMandal) {
        toast.error("You do not have permission to delete mandals.");
        return false;
      }
      try {
        await axios.delete(`/admin/divisions/${id}/`);
        if (selectedDistrictId) {
          await mutateMandals();
        }
        return true;
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "Failed to delete mandal";
        toast.error(message);
        console.error("Error deleting mandal:", error);
        return false;
      }
    },
    [axios, mutateMandals, canCreateMandal, selectedDistrictId]
  );

  const clearErrors = (): void => {
    setCreateStateError(null);
    setCreateDistrictError(null);
    setCreateMandalError(null);
  };

  useEffect(() => {
    /* countries static */
  }, []);

  const refreshCountries = doRefreshCountries;
  const refreshStates = doRefreshStates;
  const refreshDistricts = doRefreshDistricts;
  const refreshMandals = doRefreshMandals;

  return {
    countries,
    states,
    districts,
    mandals,

    isLoadingCountries,
    isLoadingStates,
    isLoadingDistricts,
    isLoadingMandals,
    isCreatingState,
    isCreatingDistrict,
    isCreatingMandal,

    countriesError,
    statesError,
    districtsError,
    mandalsError,
    createStateError,
    createDistrictError,
    createMandalError,

    canCreateState,
    canCreateDistrict,
    canCreateMandal,

    fetchCountries,
    fetchStates,
    fetchDistricts,
    fetchMandals,
    refreshCountries,
    refreshStates,
    refreshDistricts,
    refreshMandals,
    createState,
    createDistrict,
    createMandal,
    deleteState,
    deleteDistrict,
    deleteMandal,

    resetStates,
    resetDistricts,
    resetMandals,
    clearErrors,
  };
}
