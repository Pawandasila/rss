...
export default function ImageOverlay() {
  ...
  useEffect(() => {
    const isIcard =
      imgUrl.endsWith("icard.png") ||
      imgUrl.includes("/icard.png") ||
      imgUrl.endsWith("icard.jpg") ||
      imgUrl.includes("/icard.jpg");
    setFields((prev) => {
  ...
  function addUserImageOverlay() {
    if (!user || !user.image) {
      alert(
        "No user image found in auth. Make sure you are logged in and user.image is available."
      );
      return;
    }
  ...
  async function addQrOverlay(idValue?: string) {
    ...
    const idStr =
      idValue || prompt("Enter id to include in QR (e.g. user id)") || "";
    if (!idStr) return;
    ...
    try {
      ...
      const dataUrl = await qrcodeModule.default.toDataURL(qrValue, {
        margin: 1,
        scale: 8,
      });
      ...
      const api = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(
        qrValue
      )}`;
      const id = `qr_${Date.now()}`;
      setFields((p) => [
  ...
  async function createMergedCanvas(): Promise<{
    canvas: HTMLCanvasElement | null;
    hadImageErrors: boolean;
  }> {
  ...
  async function exportCanvasAsPNG() {
    ...
    if (hadImageErrors) {
      alert(
        "Some overlay images could not be embedded into the export due to cross-origin restrictions. Falling back to printable preview. Use the browser Print -> Save as PDF to capture the image as shown."
      );
      printDomPreview();
      return;
    }
  ...
  async function exportCanvasAsPDF() {
    ...
    if (hadImageErrors) {
      alert(
        "Some overlay images could not be embedded due to cross-origin restrictions. Opening printable preview instead."
      );
      printDomPreview();
      return;
    }
  ...
  async function exportTwoPagePDF() {
    ...
    try {
      ...
      try {
        jsPDFModule = (await import("jspdf")) as {
          jsPDF: new (options: Record<string, unknown>) => {
            addImage: (
              dataUrl: string,
              format: string,
        ...
        alert(
          "jsPDF is not installed. Run `npm install jspdf` in the frontend folder and reload."
        );
        return;
      }
  ...
  function printDomPreview(secondPageImage?: string) {
    ...
    </style></head><body><div class="page">${container.innerHTML}</div>${
      secondPageImage
        ? `<div class="page"><img src="${secondPageImage}" style="max-width:100%;height:auto;display:block;margin:0 auto"/></div>`
        : ""
    }</body></html>`;
    ...
    <div className="space-y-4">
      <div className="flex gap-2 items-center">
        ...
        <button
          ...
          onClick={addUserImageOverlay}
        >
          Add my image
        </button>
        <button
          className="bg-indigo-600 text-white px-3 py-2 rounded"
          onClick={() => addQrOverlay(user?.id?.toString())}
        >
          Add QR
        </button>
        <button
              ...
              <div className="mt-2 space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <label className="text-xs inline-flex items-center gap-1">
                    <input
                      ...
                      checked={!!f.bold}
                      onChange={(e) =>
                        updateField(f.id, { bold: e.target.checked })
                      }
                    />
                    <span>Bold</span>
                  </label>
                ...
                <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    value={f.top ?? 0}
                    onChange={(e) =>
                      updateField(f.id, { top: Number(e.target.value) })
                    }
                  />
                  ...
                  <input
                    ...
                    value={f.left ?? 0}
                    onChange={(e) =>
                      updateField(f.id, { left: Number(e.target.value) })
                    }
                  />
                </div>
                <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    value={f.fontSize ?? 14}
                    onChange={(e) =>
                      updateField(f.id, { fontSize: Number(e.target.value) })
                    }
                  />
                ...
                {f.imageSrc && (
                  <div className="flex gap-2 mt-1 items-center">
                    ...
                    <input
                      ...
                      value={f.width ?? 100}
                      onChange={(e) =>
                        updateField(f.id, { width: Number(e.target.value) })
                      }
                    />
                    ...
                    <input
                      ...
                      value={f.height ?? 100}
                      onChange={(e) =>
                        updateField(f.id, { height: Number(e.target.value) })
                      }
                    />
                  </div>
                )}
