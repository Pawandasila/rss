...
export function proxy(request: NextRequest) {
  ...
  const redirectCount = parseInt(
    request.cookies.get("_redirect_count")?.value || "0",
    10
  );
  ...
  searchParams.forEach((value, key) => {
    if (containsXSSPattern(value)) {
      console.warn(
        `[Security] Potentially malicious parameter detected: ${key}=${value}`
      );
  ...
  if (isSensitiveRoute(pathname)) {
    ...
    const ip =
      ...
      "unknown";
    console.log(
      `[Audit] Access to ${pathname} from IP: ${ip}, User-Agent: ${userAgent.substring(
        0,
        50
      )}`
    );
